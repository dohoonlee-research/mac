...
@persistence.persistent_class
class SongUNetModified(torch.nn.Module):
    def __init__(self,
        img_resolution,                     # Image resolution at input/output.
        in_channels,                        # Number of color channels at input.
        out_channels,                       # Number of color channels at output.
    ...
    def forward(self, x, class_labels):
        # Mapping.
        if self.map_label is not None:
            emb = self.map_label(class_labels * np.sqrt(self.map_label.in_features))
            emb = silu(self.map_layer0(emb))
            emb = silu(self.map_layer1(emb))
        else:
            emb = torch.zeros(x.shape[0], self.emb_channels).to(x.device)
...
@persistence.persistent_class
class DhariwalUNetModified(torch.nn.Module):
    def __init__(self,
        img_resolution,                     # Image resolution at input/output.
        in_channels,                        # Number of color channels at input.
        out_channels,                       # Number of color channels at output.
        ...
        channel_mult_emb    = 4,            # Multiplier for the dimensionality of the embedding vector.
        num_blocks          = 4,            # Number of residual blocks per resolution.
        attn_resolutions    = [16,16],    # List of resolutions with self-attention.
    ...
    def forward(self, x, class_labels):
        ...
        if self.map_label is not None:
            ...
            emb = torch.zeros(x.size(0), self.emb_channels).to(x.device)
...
class EDMPrecond(torch.nn.Module):
    ...
    def forward(self, x, sigma, gamma_1, class_labels=None, force_fp32=False, **model_kwargs):
        x = x.to(torch.float32)
        sigma = sigma.to(torch.float32).reshape(-1, 1, 1, 1)
        class_labels = None if self.label_dim == 0 else torch.zeros([1, self.label_dim], device=x.device) if class_labels is None else class_labels.to(torch.float32).reshape(-1, self.label_dim)
        dtype = torch.float16 if (self.use_fp16 and not force_fp32 and x.device.type == 'cuda') else torch.float32
...
@persistence.persistent_class
class EDMgammaPrecond(torch.nn.Module):
    def __init__(self,
        img_resolution,                     # Image resolution.
        img_channels,                       # Number of color channels.
        label_dim       = 0,                # Number of class labels, 0 = unconditional.
    ...
    def forward(self, x, sigma, gamma_1, class_labels=None, force_fp32=False, **model_kwargs):
        ...
        c_skip = self.sigma_data ** 2 / (gamma_1 ** 2 + self.sigma_data ** 2)
        c_out = gamma_1 * self.sigma_data / (gamma_1 ** 2 + self.sigma_data ** 2).sqrt()
        c_in = 1 / (self.sigma_data ** 2 + gamma_1 ** 2).sqrt()
        c_noise = sigma.log() / 4
        gamma_1 = gamma_1.log() / 4

        F_x = self.model(
            torch.cat(((c_in * x).to(dtype), gamma_1.to(dtype)), dim=1), 
